...
import { usePlaybooks, PlaybookEntry, PlaybookRule, IPlaybook } from '@/hooks/usePlaybooks';
...
const PlaybookTab = () => {
  ...
  const onSubmit = async (data: PlaybookFormData) => {
    try {
      ...
      const newPlaybook: Omit<IPlaybook, 'id'> = {
        name: data.name,
  ...
  const extendPlaybookToEntry = (playbook: IPlaybook): PlaybookEntry => {
    return {
      ...playbook,
      entryConditions: [],
      exitConditions: []
        ...
        <TabsContent value="my-playbooks" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlaybooks.map((playbook) => (
              <PlaybookCard
                key={playbook.id}
                playbook={extendPlaybookToEntry(playbook)}
