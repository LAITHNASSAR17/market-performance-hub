...
const MentorPerformanceChart = ({ acceptedMentees }) => {
  // Generate performance data with a limited dataset
  const generatePerformanceData = () => {
    const data = [];
    const today = new Date();
    
    // Limit to just 15 days instead of 30 to reduce data size
    for (let i = 15; i >= 0; i--) {
      const date = new Date();
      ...
      acceptedMentees.slice(0, MAX_MENTEES_CHART).forEach((mentee, index) => {
        const previous = i < 15 ? data[data.length - 1]?.[`mentee${index}`] || 0 : 0;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Cumulative P&L over the last 15 days for up to {MAX_MENTEES_CHART} mentees
        </CardDescription>
      </CardHeader>
      <CardContent>
...
// MenteeInviteForm component
const MenteeInviteForm = ({ onInvite, emailInput, setEmailInput, isOpen, setIsOpen }) => {
        ...
        <Tabs defaultValue="email">
          ...
          <TabsContent value="email" className="mt-4">
            ...
            <DialogFooter className="mt-4">
              <Button onClick={onInvite} disabled={!emailInput.trim()}>
...
// MenteeStatsTable component
const MenteeStatsTable = ({ mentees, loading, onSwitchToMenteeView, onNavigateToMenteeNotes, onCancelMentorship }) => {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        {loading ? (
                      ...
                      <div className="flex justify-end space-x-2">
                        {mentee.status === 'accepted' && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => onSwitchToMenteeView(mentee.id)}
                        ...
                        {mentee.status === 'accepted' && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => onNavigateToMenteeNotes(mentee.id)}
                            title="Mentor notes"
                          >
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => onCancelMentorship(mentee.mentorshipId)}
          ...
          <div className="py-8 text-center flex flex-col items-center">
            ...
            <Button onClick={() => onSwitchToMenteeView()}>
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 gap-6">
        <MentorPerformanceChart acceptedMentees={acceptedMentees} />
        
        <MenteeStatsTable 
          mentees={mentees}
          loading={loading}
          onSwitchToMenteeView={switchToMenteeView}
          onNavigateToMenteeNotes={handleNavigateToMenteeNotes}
          onCancelMentorship={cancelMentorship}
        />
      </div>
